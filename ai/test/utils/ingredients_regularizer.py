...
from regularizer_prompts import *
...
class IngredientsRegularizer:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError("OpenAI API 키가 필요합니다. 환경변수 OPENAI_API_KEY를 설정하거나 초기화시 제공하세요.")
    ...
    async def batch_normalize_with_openai(self, ingredients: List[str], batch_size: int = 20) -> Dict[str, str]:
        ...
        for batch in batches:
            ...
            # 남은 항목이 없으면 다음 배치로
            if not uncached:
                results.update({i: self.normalized_cache[i] for i in batch if i in self.normalized_cache})
                continue
