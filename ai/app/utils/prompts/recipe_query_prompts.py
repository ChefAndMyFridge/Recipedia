...
12. 주재료가 주류(소주, 맥주, 와인 등)만 있을 경우, 해당 주류와 함께 먹기 좋은 안주를 추천하세요.
    - 단, 안주를 추천할 때에도 냉장고 재료만을 활용하여 추천해야합니다.
    - 예: 주재료 [소주] → 부대찌개 (한식), 회 (한식), 삼겹살 (한식)
    - 예: 주재료 [맥주] → 치킨 (양식/한식), 감자튀김 (양식), 소시지 (양식)
    - 예: 주재료 [와인] → 치즈플레터 (양식), 브루스케타 (이탈리아), 카르파초 (이탈리아)

    각 주류별 특성에 맞는 전통적인 안주 조합을 추천하고, 다양한 국가의 안주 문화를 균형있게 반영하세요.
    안주 추천시에도 주재료가 없는 것처럼 취급하고, 냉장고 재료만을 활용하여 추천해야합니다.
