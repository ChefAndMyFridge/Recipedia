...
// 고내에 저장된 재료 목록 조회
export const getIngredientsApi = async (location: string, sort: string): Promise<Ingredients[]> => {
  try {
    const response = await instance.get(`v1/ingredient?storage=${location}&sort=${sort}`);
    console.log(`v1/ingredient?storage=${location}&sort=${sort}`, response.data);
