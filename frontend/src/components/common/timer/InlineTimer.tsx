...
const InlineTimer = ({ defaultTimer, timerStep, currentTimer, isRunning, onTimerUpdate }: InlineTimerProps) => {
  // 현재 타이머 값
    ...
    <>
      <div className="w-full flex flex-col gap-3 items-center justify-between font-preSemiBold">
        <div className="flex w-full justify-between items-center">
          ...
          <button onClick={handleRunTimer} className="px-2 py-1 rounded-full">
            {isRunning ? <IconPause width={11} height={11} /> : <IconPlay width={11} height={11} />}
          </button>
          <button onClick={handleResetTimer}>
            <IconStop width={10} height={10} />
          </button>
        </div>
